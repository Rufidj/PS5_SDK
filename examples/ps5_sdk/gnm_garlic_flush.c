...
/*
 * gnm_garlic_flush — gnm_onion_probe + sceGnmFlushGarlic tras el fence.
 *
 * gnm_onion_probe confirmo: fence=BEEF0002, GPU ejecuta DMA+WRITE_DATA
 * correctamente con CB en ONION. Pantalla negra → datos en GPU L2.
 *
 * Unica diferencia aqui: llamamos sceGnmFlushGarlic() DESPUES de que
 * el fence confirma que el DMA termino, para forzar flush GPU L2 → DRAM.
 * El display engine lee de DRAM → si ROJO, el problema era solo cache.
 *
 * Si sigue negro: el DMA escribe en GPU VA que NO es el fisico del FB.
 * Proximo paso seria usar ONION para el framebuffer (CPU VA == GPU VA).
 */
...
void _start(u64 eboot_base, u64 dlsym_addr, struct ext_args *ext) {
...

    {
        ...
        *fence = 0u;
        ...
        u64 fb0_va   = (u64)fbs[0];
        u32 fbsz     = FW * FH * 4u;

        /* Build PM4 in ONION CB (identical to gnm_onion_probe) */
        u32 *pm4 = (u32 *)cb_mem;
        u32  n   = 0;

        pm4[n++] = PM4_WD_HDR; pm4[n++] = WD_TC_L2;
        pm4[n++] = (u32)(fence_va & 0xFFFFFFFFu);
        pm4[n++] = (u32)(fence_va >> 32);
        pm4[n++] = 0xBEEF0001u;

        pm4[n++] = PM4_DMA_HDR; pm4[n++] = DMA_FILL;
        pm4[n++] = 0x00FF0000u;     /* RED */
        pm4[n++] = 0u;
        ...
        /* Wait for fence BEEF0002 (both WD + DMA confirmed done) */
        for (u32 i = 0; i < 2000u && *fence != 0xBEEF0002u; i++) {
            if (ex.usleep_fn) NC(ex.G, ex.usleep_fn, 1000, 0, 0, 0, 0, 0);
        }
        fence_val = *fence;

        /* *** NUEVA LINEA: flush GPU L2 → DRAM antes del flip *** */
