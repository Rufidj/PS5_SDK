...
static s32 show_text_page(struct ps5_example *ex, u8 *dialog, const char *fmt, ...) {
    ...
    ps5_sdk_vsnprintf((char *)(dialog + 0xE0), 160, fmt, ap);
    __builtin_va_end(ap);
    ex_setup_user_msg(dialog, dialog + 0x88, (char *)(dialog + 0xE0), ex->user_id);
    ret = (s32)NC(ex->G, ex->dlg_open, (u64)dialog, 0, 0, 0, 0, 0);
